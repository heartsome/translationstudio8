...
/**
 * The TableCombo class represents a selectable user interface object that combines a label, textfield, and a table and
 * issues notification when an item is selected from the table. Note: This widget is basically a extension of the CCombo
 * widget. The list control was replaced by a Table control and a Label control was added so that images can be
 * displayed when a value from the drop down items has a image associated to it.
 * <p>
 * TableCombo was written to allow the user to be able to display multiple columns of data in the "Drop Down" portion of
 * the combo.
 * </p>
 * <p>
 * Special Note: Although this class is a subclass of <code>Composite</code>, it does not make sense to add children to
...
public class TableCombo extends Composite {
	 ...
	 * If the text field is editable, this has the effect of placing the i-beam at the start of the text.
	 * <p>
	...
	/**
	 * Defines what columns the drop down table will have. Use this method when you don't care about the width of the
	 * columns but want to set the column header text.
	 * @param columnHeaders
	 * @param columnWidths
	 */
	...
	/**
	 * Defines what columns the drop down table will have. Use this method when you don't care about the column header
	 * text but you want the fields to be a specific width.
	 * @param columnHeaders
	 * @param columnBounds
	 */
	...
	/**
	 * Defines what columns the drop down table will have. Use this method when you don't care about the column headers
	 * and you want the columns to be automatically sized based upon their content.
	 * @param columnHeaders
	 * @param columnWidths
	 */
	...
	/**
	 * Defines what columns the drop down table will have. Use this method when you want to specify the column header
	 * text and the column widths.
	 * @param columnHeaders
	 * @param columnBounds
	 */
	...
	/**
	 * Sets the table width percentage in relation to the width of the label control. The default value if 100% which
	 * means that it will be the same size as the label control. If you want the table to be wider than the label then
	...
	/**
	 * Sets the zero-relative column index that will be used to display the currently selected item in the label
	 * control.
	...
	/*
	 * Return the lowercase of the first non-'&' character following an '&' character in the given string. If there are
	 * no '&' characters in the given string, return '\0'.
	 */
	...
	/**
	 * returns the Table reference. NOTE: the access is public for now but will most likely be changed in a future
	 * release.
	 * @return
	 */
	...
	/**
	 * adjusts the last table column width to fit inside of the table if the table column data does not fill out the
	 * table area.
	 */
	private void autoAdjustColumnWidthsIfNeeded(TableColumn[] tableColumns, int totalAvailWidth,
		...
		if (totalAvailWidth > totalColumnWidthUsage + scrollBarSize) {
			...
			if (totalAmtToBeAllocated > 0) {
				tableColumns[totalColumns - 1].setWidth(tableColumns[totalColumns - 1].getWidth()
						+ totalAmtToBeAllocated);
			}
		}
	}
