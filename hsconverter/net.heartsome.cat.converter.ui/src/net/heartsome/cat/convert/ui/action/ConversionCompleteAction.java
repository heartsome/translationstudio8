...
public class ConversionCompleteAction extends Action {
	...
	public void run() {
		...
		if (status.getSeverity() == IStatus.ERROR) {
			...
			if (xliffFile != null) {
				IEditorDescriptor editorDescriptor = PlatformUI.getWorkbench().getEditorRegistry().findEditor(
						XLIFF_EDITOR_ID);
				if (editorDescriptor != null) {
					try {
						IDE.openEditor(page, new File(xliffFile).toURI(), XLIFF_EDITOR_ID, true);
					} catch (PartInitException e) {
						MessageDialog.openInformation(shell, "提示", "转换完成！但是在尝试打开该 XLIFF 文件时发生异常：\n" + e.getMessage());
						e.printStackTrace();
					}
				}
			} else {
