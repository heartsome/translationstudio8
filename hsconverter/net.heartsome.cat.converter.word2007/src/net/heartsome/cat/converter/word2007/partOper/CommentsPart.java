...
/**
 * 批注文件 comments.xml 的处理 
 * @author robert
 */
public class CommentsPart extends PartOperate {
	/**
	 * 正向转换所用到的构造方法
	 * @param partPath
	 * @param xlfOutput
	 ...
	 * @throws Exception
	 */
	public CommentsPart(String partPath, XliffOutputer xlfOutput, StringSegmenter segmenter) throws Exception {
	 ...
	 * @param xlfInput
	 */
	public CommentsPart (String partPath, XliffInputer xlfInput) throws Exception {
	...
	protected void converter() throws Exception {
		operateTransAttributes("/w:comments/w:comment/descendant::w:p/w:r/w:pict/v:shape/@alt");
	}
	 ...
	 * @throws Exception
	 */
	protected void operateCommentsContent(String id) throws Exception {
		
		// 处理的单元为 w:p
		String xpath = "/w:comments/w:comment[@w:id='" + id + "']/descendant::w:p";
...
// ------------------------------------------  下面是逆转换的代码 ------------------------------------------------
	...
	protected void reverseConvert() throws Exception {
		// 处理的单元为 w:p
		String xpath = "/w:comments/w:comment/descendant::w:p";
		ap.selectXPath(xpath);
		while(ap.evalXPath() != -1){
			// DOLATER 先处理所有的属性
		...
		//再处理可翻译属性
		xpath = "/w:comments/w:comment/descendant::w:p/w:r/w:pict/v:shape/@alt";
