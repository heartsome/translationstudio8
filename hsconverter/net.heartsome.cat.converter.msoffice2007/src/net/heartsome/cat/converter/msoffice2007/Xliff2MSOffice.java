...
public class Xliff2MSOffice implements Converter {
	...
	public static final String NAME_VALUE = "XLIFF to MS Office 2007 Conveter";
	...
	class Xliff2MSOfficeImpl {
		...
		public Map<String, String> run(Map<String, String> args, IProgressMonitor monitor) throws ConverterException {
			...
			try {
				...
				monitor.beginTask(Messages.getString("msoffice2007.Xliff2MSOffice.task1"), 20);
				...
				try {
					...
					separateMonitor.beginTask(Messages.getString("msoffice2007.Xliff2MSOffice.task2"), files.size());
				...
				if (isInfoEnabled) {
					...
					LOGGER.info(Messages.getString("msoffice2007.Xliff2MSOffice.logger2"), endTime);
					LOGGER.info(Messages.getString("msoffice2007.Xliff2MSOffice.logger3"), endTime - startTime);
				}

				monitor.subTask(Messages.getString("msoffice2007.Xliff2MSOffice.task3"));
				if (isInfoEnabled) {
					startTime = System.currentTimeMillis();
					LOGGER.info(Messages.getString("msoffice2007.Xliff2MSOffice.logger4"), startTime);
				}
				...
				if (isInfoEnabled) {
					endTime = System.currentTimeMillis();
					LOGGER.info(Messages.getString("msoffice2007.Xliff2MSOffice.logger5"), endTime);
					LOGGER.info(Messages.getString("msoffice2007.Xliff2MSOffice.logger6"), endTime - startTime);
				}
				monitor.worked(2);
				...
				try {
					...
					conversionMonitor.beginTask(Messages.getString("msoffice2007.Xliff2MSOffice.task4"), totalTask);
					if (isInfoEnabled) {
						startTime = System.currentTimeMillis();
						LOGGER.info(Messages.getString("msoffice2007.Xliff2MSOffice.logger7"), startTime);
					}
					...
					while ((entry = in.getNextEntry()) != null) {
						...
						String messagePattern = Messages.getString("msoffice2007.Xliff2MSOffice.msg1");
						...
						if (entry.getName().matches(".*\\.[xX][mM][lL]\\.skl")) { //$NON-NLS-1$
							...
							if (res.get(Converter.ATTR_TARGET_FILE) == null) {
								ConverterUtils.throwConverterException(Activator.PLUGIN_ID,
										Messages.getString("msoffice2007.Xliff2MSOffice.msg2"));
							}
				...
				if (isInfoEnabled) {
					endTime = System.currentTimeMillis();
					LOGGER.info(Messages.getString("msoffice2007.Xliff2MSOffice.logger8"), endTime);
					LOGGER.info(Messages.getString("msoffice2007.Xliff2MSOffice.logger9"), endTime - startTime);
				}
				...
				ConverterUtils.throwConverterException(Activator.PLUGIN_ID, Messages.getString("msoffice2007.Xliff2MSOffice.msg3"),
						e);
			} finally {
				monitor.done();
			}
